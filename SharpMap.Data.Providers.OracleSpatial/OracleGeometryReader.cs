...
namespace SharpMap.Data.Providers.OracleSpatial
{
    public class OracleGeometryReader
    {
        ...
        private static List<Coordinate> Coordinates(int dim, int lrs, int gtemplate, Decimal[] ordinates)
        {
            ...
            var length = (len == 0 ? 0 : ordinates.Length / len);
                                                    ...
                                                    List<Coordinate> coords, int numGeom)
        {
            ...
            var length = coords.Count * dim;

            if (!(sOffset <= length))
                throw new ArgumentException("ELEM_INFO STARTING_OFFSET " + sOffset +
                                            " inconsistent with ORDINATES length " + coords.Count);

            var endTriplet = (numGeom != -1) ? elemIndex + numGeom : elemInfo.Length / 3 + 1;

            var list = new List<IGeometry>();
            IGeometry geom = null;

            var cont = true;
            for (var i = elemIndex; cont && i < endTriplet; i++)
            {
                var etype = EType(elemInfo, i);
                var interpretation = Interpretation(elemInfo, i);
                                                ...
                                                List<Coordinate> coords, int numGeom)
        {
            ...
            for (var i = elemIndex; cont && i < endTriplet && (etype = EType(elemInfo, i)) != SdoEType.Unknown; i++)
            {
                if ((etype == SdoEType.Polygon) || (etype == SdoEType.PolygonExterior))
                {
                                                ...
                                                List<Coordinate> coords, int numGeom)
        {
            ...
            var endTriplet = (numGeom != -1) ? (elemIndex + numGeom) : (elemInfo.Length / 3);
        ...
        private IPolygon CreatePolygon(int dim, int lrs, Decimal[] elemInfo, int elemIndex, List<Coordinate> coords)
        {
            ...
            var cont = true;
            for (var i = elemIndex + 1; cont && (etype = EType(elemInfo, i)) != SdoEType.Unknown; i++)
            {
        ...
        sOffset = StartingOffset(elemInfo, elemIndex);
            ...
            var len = (dim + lrs);
            var start = (sOffset - 1) / len;
            var eOffset = StartingOffset(elemInfo, elemIndex + 1); // -1 for end
            var end = (eOffset != -1) ? ((eOffset - 1) / len) : coords.Count;
        ...
        private static Coordinate[] ToPointArray(ICollection<Coordinate> input)
        {
            ...
            pts.AddRange(input.Select(point => new Coordinate(point.X, point.Y, point.Y)));
